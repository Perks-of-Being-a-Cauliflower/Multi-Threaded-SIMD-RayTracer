...
struct Colour8
{
	...
	// create initialised colour SIMD
	__forceinline Colour8(__m256 r, __m256 g, __m256 b)
	{
		reds = r;
		greens = g;
		blues = b;
	...
	// create initialised colour
	__forceinline Colour8(float r, float g, float b)
	{
		reds = _mm256_set1_ps(r);
		greens = _mm256_set1_ps(g);
		blues = _mm256_set1_ps(b);
	}

	// create colour from integer (pixel stored in 0x00BBGGRR format)
	
	inline Colour8(__m256 value) :
		red(__m256(_mm256_div_ps(_mm256_and_ps(value, _mm256_set1_ps(0xFF)), _mm256_set1_ps(255.0f))),
		green(__m256((value >> 8) & 0xFF) / 255.0f),
		blue(__m256((value >> 16) & 0xFF) / 255.0f)
	{
	}
	...
	{
		this->reds = _mm256_add_ps(c2.reds, this->reds);
		this->greens = _mm256_add_ps(c2.greens, this->greens);
		this->blues = _mm256_add_ps(c2.blues, this->blues);
		return *this;
	}
};
*/
