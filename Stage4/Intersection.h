...
typedef struct Intersection8
{
	...
	Point8 pos;											// point of intersection
...
// test to see if collision between ray and a box happens before time t (equivalent to distance)
// updates closest collision time (/distance) if collision occurs
bool isBoxIntersected(const Scene* b, const Ray* r, float* t, int* index);

bool isBoxIntersectedShadow(const Scene* b, const Ray* r, float* t);
