...
bool isBoxIntersected(const Scene* s, const Ray* r, float* t, int* index)
{
	...
	// outer loop for each 8 values 
	for (unsigned int i = 0; i < s->numBoxesSIMD; ++i)
	{
		// inner loop for each value within the simd
		for (unsigned int j = 0; j < 8; ++j)
		{
			// calculate distances to each "close" and "far" face, check the article for the rationale
			// set points from simd SOA at the i element accessing each element in the simd through j
			Point p0 = { s->boxP1x[i].m256_f32[j], s->boxP1y[i].m256_f32[j], s->boxP1z[i].m256_f32[j] };
			Point p1 = { s->boxP2x[i].m256_f32[j], s->boxP2y[i].m256_f32[j], s->boxP2z[i].m256_f32[j] };

			Vector t0 = ((p0 - r->start) / r->dir);
			Vector t1 = ((p1 - r->start) / r->dir);
			...
			if (tmin < tmax) {
				...
				{
					*t = tmin;
					// update index so it gets the correct value from the extra loop
					*index = (i * 8) + j;
					returnedVal = true;
				}
			}
