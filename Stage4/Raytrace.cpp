...
void simdifySceneContainers(Scene& scene)
{
	...
	else
	{
		...
		{
			// don't let the source index extend out of the AoS array
			// i.e. copy the last value into the extra array slots when numBoxes isn't exactly divisible by 8
			// pretty lazy way to fix this, but it works
...
void applyLightingSIMD(Scene &scene, Ray& viewRay, Intersection& intersect) {
	// helper size (so we don't just have 8 everywhere)
	unsigned int valuesPerVector = sizeof(__m256) / sizeof(float);
	...
	else
	{
		...
		scene.lightX = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		scene.lightY = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		scene.lightZ = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		...
		for (unsigned int i = 0; i < scene.numLightsSIMD * valuesPerVector; ++i)
		{
			...
			// set light values to null if numlights limit is reached
			if (i >= scene.numLights) {
				scene.red[i / valuesPerVector].m256_f32[i % valuesPerVector] = NULL;
				scene.green[i / valuesPerVector].m256_f32[i % valuesPerVector] = NULL;
				scene.blue[i / valuesPerVector].m256_f32[i % valuesPerVector] = NULL;
				scene.lightX[i / valuesPerVector].m256_f32[i % valuesPerVector] = NULL;
				scene.lightY[i / valuesPerVector].m256_f32[i % valuesPerVector] = NULL;
				scene.lightZ[i / valuesPerVector].m256_f32[i % valuesPerVector] = NULL;
			} else {
				...
				scene.lightX[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].pos.x;
				scene.lightY[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].pos.y;
				scene.lightZ[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].pos.z;
			}
...
int main(int argc, char* argv[])
{
	...
	applyLightingSIMD(scene, viewRay, intersect);
